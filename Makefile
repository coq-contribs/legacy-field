...
OCAMLLIBS?=-I "${COQLIB}/user-contrib/LegacyRing"\
  -I "."
COQLIBS?=\
  -R "." LegacyField\
  -I "${COQLIB}/user-contrib/LegacyRing"\
  -I "."
COQDOCLIBS?=\
  -R "." LegacyField
...
install-natdynlink:
	cd "." && for i in $(CMXSFILES); do \
	 ...
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/LegacyField/$$i; \
	done
...
install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/LegacyField/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/LegacyField/$$i;\
	done
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/LegacyField && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "LegacyField" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL)/LegacyField \\\n' >> "$@"
	printf '&& rm -f $(shell find "html" -maxdepth 1 -and -type f -print)\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL) && find LegacyField/html -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	chmod +x $@
